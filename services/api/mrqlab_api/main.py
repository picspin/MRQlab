...
from mrqlab_experiment.sequence_compose import ComposeSequenceRequest, compose_sequence
...
@app.post("/sequences/compose", response_model=SequenceIR)
def sequences_compose(request: ComposeSequenceRequest):
    try:
        return compose_sequence(request)
    except (ValueError, TypeError) as exc:
        raise HTTPException(422, str(exc)) from exc
...
@app.post("/pulse/inspect")
def api_pulse_inspect(req: PulseInspectRequest):
    """v0.49: sinc B1 / freq response / slice profile / EPG 3x3 — backend is the only pulse inspector."""
    return inspect_pulse_dict(req)
