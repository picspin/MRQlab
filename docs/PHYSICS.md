...
| EPG-X | yes | hard_rf, configuration_states, exchange, multi_pool | Two-pool liquid Bloch–McConnell evolution on the 6-row layout, or MT evolution on the 4-row free/bound layout. A declared `metadata["cest"]` CW offset sweep on two liquid pools produces a single-voxel `z_spectrum`; Super-Lorentzian absorption remains MT-bound-pool-only and on-resonance Super-Lorentzian remains closed. |
