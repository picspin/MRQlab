...
MRQLab is a teaching MRI simulator, not a clinical scanner, safety simulator, or hardware controller. The experiment kernel selects a `StateRepresentation` and applies typed `PhysicsOperator` records. `SequenceIR` remains the only scanner-level event source. The physics microkernel converts its RF, gradient, ADC-gate, and NCO channels into `RfOp`, `Relax`, `Shift`, `GradInterval`, and `AdcSample`; a declared CEST sweep is a `SaturationOp` on the EPG-X two-liquid layout, not a hard `RfOp`. Plugins apply those operators to their own state.
...
| EPG-X | yes | hard_rf, configuration_states, exchange, multi_pool | Two-pool liquid Bloch–McConnell evolution on the 6-row layout, or MT evolution on the 4-row free/bound layout. A declared `metadata["cest"]` CW or rectangular pulsed-train offset sweep on two liquid pools produces a single-voxel `z_spectrum`; Super-Lorentzian absorption remains MT-bound-pool-only and on-resonance Super-Lorentzian remains closed. |
